...
productController.createNewProduct = async (req, res) => {
    try {
        ...
        if (!req.files?.length)
            throw new Error_1.default(Error_1.HttpCode.INTERNAL_SERVER_ERROR, Error_1.Message.CREATE_FAILED);
        const data = req.body;
        data.productImages = req.files?.map(ele => {
            return ele.path.replace(/\\/g, "/");
        });
        await productService.createNewProduct(data);
        res.send(`<script> alert("Successfuly creation!"); window.location.replace('/admin/product/all') </script>`);
    }
    catch (err) {
